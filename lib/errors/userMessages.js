...
const CODE_MESSAGES = {
  ...
  TEAM_MANDATORY_FULL: "Mandatory roster is already full. You can still add substitutes if slots remain.",
  TEAM_SUBSTITUTE_LIMIT: "No substitute slots left for this team.",
  TEAM_REQUIRED_MEMBERS_NOT_MET: "Add the required members before this team can be completed.",
  LEADER_ENTRY_NOT_ALLOWED: "This event requires members to join themselves via invite.",
  ROSTER_INVALID: "Check the member details and try again.",
...
/**
 * Payment exists in CREATED — may be unpaid checkout or awaiting webhook.
 * Reconcile via POST /payments/{id}/sync before trusting UI.
 */
export function needsPaymentSync(registration) {
  if (!registration?.payment?.id) return false;
  return String(registration.payment.status || "").toUpperCase() === "CREATED";
}

/**
 * Allow continue/retry when unpaid.
 * CREATED: checkout may have been dismissed — sync first, then allow pay again.
 * FAILED: retry allowed.
 * Confirmed/PAID: never retry.
 */
export function canRetryPayment(registration) {
  ...
  return status === "FAILED" || status === "CREATED" || status === "";
}

/** @deprecated Prefer needsPaymentSync + canRetryPayment */
export function isPaymentProcessing(registration) {
  return needsPaymentSync(registration);
}
